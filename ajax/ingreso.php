	...
	case 'listar':
		$rspta=$ingreso->listar();
 		...
 		while ($reg=$rspta->fetch_object()){
 			...
 			$data[]=array(
 				"0"=>(($reg->estado=='Aceptado')?'<button class="btn btn-primary" onclick="mostrar('.$reg->idingreso.')"><i class="fa fa-eye"></i></button>'.
 					' <button class="btn btn-danger" onclick="anular('.$reg->idingreso.')"><i class="fa fa-close"></i></button>':
 					'<button class="btn btn-primary" onclick="mostrar('.$reg->idingreso.')"><i class="fa fa-eye"></i></button>').
		...
		$rspta=$articulo->listarActivos();
 		...
 		while ($reg=$rspta->fetch_object()){
 			$data[]=array(
 				...
 				"3"=>$reg->codigo,
 				"4"=>$reg->stock,
 				"5"=>"<img src='../files/articulos/".$reg->imagen."' height='100px' width='100px' >"
 				);
 		}
 		$results = array(
 			"sEcho"=>1, //Información para el datatables
