              ...
              <div class="col-md-12">
                  <div class="box">
                    <div class="box-header with-border">
                          <h1 class="box-title">Sistema de Cocina <button class="btn btn-info" id="btnagregar" onclick="location.reload();"><i class="fa fa-refresh"></i> Actualizar</button></h1>
...
<body>
    ...
    <script>
        ...
        async function cargarPedidos() {
            try {
                const response = await fetch('../modelos/obtener_pedidos.php');
                const pedidos = await response.json();

                // Agrupar pedidos por idventa
                const pedidosAgrupados = pedidos.reduce((acc, detalle) => {
                    if (!acc[detalle.idventa]) {
                        acc[detalle.idventa] = { idventa: detalle.idventa, detalles: [] };
                    }
                ...
                pedidosArray.forEach(pedido => {
                    ...
                    pedidoHTML.innerHTML = `
                        ...
                        <button class="btn btn-success" onclick="actualizarEstado(${pedido.idventa}, 'Listo')">Listo</button>
                    `;
